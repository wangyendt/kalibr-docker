...
def write_cam_imu_report(
    ...
    data = {
        ...
        "imu_yaml": str(imu_yamls[0]) if len(imu_yamls) == 1 else None,
        "imu_yamls": [str(path) for path in imu_yamls],
        "args": command_args,
    ...
    for index, imu_yaml in enumerate(imu_yamls):
        lines.append(f"- IMU YAML {index}: `{imu_yaml}`")
    lines.extend([
        "",
        "## Messages",
        "",
