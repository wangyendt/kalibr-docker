...
这个仓库适合交给外部用户或产线脚本调用：不要求安装 ROS，不要求理解 Kalibr 的工作空间结构，也不要求手工翻长日志。
...
ProductionCalibration 兼容的 cam-IMU corner-file 模式：
...
- `cam-imu` 保留 corner-file 与 H5 两条离线路径，能对齐 ProductionCalibration 的常用参数。
...
该子工程把 Kalibr cam-IMU 推导成书，并用 Ceres 复现独立求解链路。12 组 production 数据上，Ceres 原生二进制平均墙钟约 `103 s`，Kalibr Docker 基线约 `203 s`；严格倍率需要在同一 native Linux 环境复测。实验入口见 `kalibr-camimu-ceres/docs/experiment/20260616_Ceres与KalibrDocker多数据集速度精度对比.md`。
...
- ProductionCalibration-compatible offline `cam-imu` inputs.
