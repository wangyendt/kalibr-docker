...
用**两种初始化口径**对比 `ceres_cam_imu` 和 Kalibr。两表的**优化器完全相同,唯一变量是初始化**,这样可以把"初始化的影响"和"两套实现本身的差异"分开。2026-06-17 按当前代码重跑 12 套 production 数据后,表1 不再是"全部充分收敛"的成功样本:3 月 6 套 `independent-joint-full` 全部触发 150 步上限并跑到坏局部最小;表2 热启动仍稳定收敛。因此下面的 current rerun 同时是精度/速度更新,也是一次独立初始化回归记录。

- **表1 — 独立(完全不参考 Kalibr 初始化)**:不读 Kalibr 任何输出,复刻 Kalibr 自己的初始化(`--estimate-time-shift-prior` gyro-norm 互相关 + `--estimate-orientation-gravity-prior` Wahba/Kabsch 闭式 + 外参平移从 0 起 + pose 样条拟合 + bias 零)。这是"从零独立标定"的真实能力,对应 preset `independent-joint-full`,已固化在代码;本轮结果说明当前独立初始化链路存在生产级回归。
...
| Ceres | `ceres_cam_imu/build/calibrate_cam_imu`,原生 arm64 Release,preset `independent-joint-full` + `--init-from-kalibr` 热启动变体 |
...
Ceres 从零独立初始化(`independent-joint-full`)。差异列为 Ceres 减 Kalibr(定义见上节)。reproj 为各自收敛后的重投影均值。墙钟列括号内是收敛用的迭代数(Ceres 单次全联合 LM;Kalibr Optimizer2 LM);`NC` 表示 Ceres `NO_CONVERGENCE`。本轮输出在 `ceres_cam_imu/out/ceres_sweeps/prod_current_rerun_<dataset>/summary.csv`。

| 数据集 | 外参平移差 | 外参旋转差 | time-shift 差 | reproj px (Ceres/Kalibr) | Ceres 墙钟 | Kalibr 墙钟 |
|---|---:|---:|---:|---|---:|---:|
| 2025_03_14_00_10_18 | 151.22 mm | 6.7708° | -2.548 ms | 28.1622 / 0.1798 | 152 s (151 it, NC) | 162 s (5 it) |
| 2025_03_14_00_34_14 | 153.70 mm | 6.0591° | -1.992 ms | 24.8395 / 0.1797 | 150 s (151 it, NC) | 215 s (6 it) |
| 2025_03_14_00_50_37 | 146.50 mm | 3.6864° | -1.928 ms | 16.3630 / 0.1801 | 153 s (151 it, NC) | 145 s (3 it) |
| 2025_03_14_02_13_45 | 145.83 mm | 2.1848° | -3.544 ms | 18.8563 / 0.1791 | 150 s (151 it, NC) | 207 s (7 it) |
| 2025_03_14_02_21_41 | 150.33 mm | 0.7090° | -1.408 ms | 15.2762 / 0.1787 | 152 s (151 it, NC) | 254 s (10 it) |
| 2025_03_14_10_23_35 | 140.17 mm | 0.2782° | +61.056 ms | 2.9750 / 0.1766 | 144 s (151 it, NC) | 168 s (4 it) |
| 2025_04_19_18_43_05 | 1.40 mm | 0.0051° | -3.199 ms | 0.1708 / 0.1715 | 84 s (79 it) | 225 s (10 it) |
| 2025_04_19_19_03_03 | 3.13 mm | 0.0134° | -6.372 ms | 0.1737 / 0.1712 | 83 s (78 it) | 246 s (10 it) |
| 2025_04_19_19_20_46 | 1.48 mm | 0.0000° | -0.700 ms | 0.1704 / 0.1706 | 81 s (77 it) | 194 s (7 it) |
| 2025_04_19_19_35_04 | 1.31 mm | 0.0000° | +1.344 ms | 0.1719 / 0.1708 | 79 s (75 it) | 183 s (8 it) |
| 2025_04_19_19_55_25 | 1.68 mm | 0.0073° | +1.420 ms | 0.1728 / 0.1717 | 88 s (84 it) | 178 s (7 it) |
| 2025_04_19_20_21_09 | 1.39 mm | 0.0118° | -5.391 ms | 0.2076 / 0.1712 | 90 s (87 it) | 261 s (12 it) |

汇总(12 套):

| 指标 | 范围 / 典型 |
|---|---|
| 外参平移差 | 全量 1.3 – 153.7 mm;March 组 140.2 – 153.7 mm,April 组 1.3 – 3.1 mm |
| 外参旋转差 | 全量 0 – 6.77°;March 组 0.278 – 6.771°,April 组 ≤ 0.014° |
| time-shift 差 | 全量 -6.37 – +61.06 ms;March 组有一套跑到 +61.06 ms |
| reproj 均值 | March 组 2.975 – 28.162 px,明显坏解;April 组 0.170 – 0.208 px |
| Ceres 墙钟 / 迭代 | 79 – 153 s / 75 – 151 it;March 组全部 151 it `NO_CONVERGENCE`,April 组全部 CONVERGENCE |
| Kalibr 墙钟 / 迭代 | 145 – 261 s / 3 – 12 it(amd64 模拟) |

这轮 current rerun 的故事和旧表不同:April 组仍然表现为正常的独立收敛,但 March 组不是"慢一点",而是统一触顶后落入坏解。旧结论里"独立口径 1.3–4.7 mm、reproj 追平 Kalibr"在当前代码上已经不能复现;生产路径不能再把 `independent-joint-full` 当作已验证稳定的零初值标定能力,需要先定位初始化/pose prior/停止条件的回归。
...
- **Ceres 和 Kalibr 不是逐位相同的优化问题。** 在弱可观的外参平移方向上,两套实现的最优点本身相差 0.19–2.63 mm(March 组 0.3–2.6 mm,April 组 0.2–0.7 mm)。差异来自权重映射(Cauchy loss)、pose 样条表示/正则、以及优化器在该弱方向上的细节,而非 bug——reproj 仍逐位相同(±0.001 px),说明这点平移差在重投影上几乎不可见。
- **表2 是当前判断实现一致性的有效口径,表1 不是。** 旧版可用"表2 < 表1"解释独立初始化的额外代价;本轮表1 March 已经是明显坏解,不能再拿来估计正常独立收敛的误差上界。表2 的意义变成:在 Kalibr 全局块附近,Ceres 前向模型、残差权重和优化器仍能收敛到 reproj 同量级、外参 mm 级邻域。
...
这轮 current rerun 不能继续用旧表的"窄迭代带"解释表1。表1 March 组墙钟 144–153 s 的主因是全部打满 151 次迭代并 `NO_CONVERGENCE`,不是正常收敛成本;April 组才是正常独立收敛的 79–90 s / 75–87 it。

表2 更适合看当前 Ceres 原生求解器的正常墙钟:12 套全部 CONVERGENCE,36–91 s / 32–89 it。Kalibr 仍是 amd64 模拟 Docker 的 145–261 s / 3–12 it,所以速度列只能说明"本机当前环境下 Ceres 原生热启动/正常收敛更快";不能把 March 表1的 150 s 当作可用生产速度,因为那是失败上限时间。
...
先确认输入口径。Ceres 原生 `calibrate_cam_imu` 只读统一中间格式:camchain/IMU/target YAML,IMU CSV,一个或多个角点 CSV,以及可选 `corner_poses` CSV;这个标定二进制本身不依赖 Kalibr Docker。`tools/prepare_ceres_inputs.py` 是外层转换入口,支持 `--source-type pkl|bag|euroc`:pkl 通过 `export_kalibr_corners.py` 转 CSV,ROS bag 通过 `export_kalibr_bag_to_ceres.py` 转 CSV,EuRoC/TUM `mav0` 目录先用 `kalibr_bagcreater` 转 bag 再走同一条 bag 导出链路。也就是说,当前 pkl/bag/euroc 的**转换/角点导出**仍复用 Kalibr Docker/ROS 环境,但**标定求解**是原生 Ceres;加 `--run-calibration` 可以一个脚本转完直接跑单阶段标定,加 `--run-two-stage` 则转完直接跑下面的 TUM parity 二阶段流程。
...
python3 ceres_cam_imu/tools/prepare_ceres_inputs.py --source-type bag \
  ...
  --out-dir <ceres_inputs> --run-two-stage -- \
  --kalibr-result <kalibr-results-imucam.txt>
```

如果输入已经转成 Ceres CSV/YAML,可直接跑新增的二阶段工具:

```bash
python3 ceres_cam_imu/tools/run_ceres_two_stage.py \
  --cam <camchain.yaml> --imu <imu.yaml> --target <april.yaml> \
  --imu-data <imu.csv> --corners <cam0_corners.csv> --corners <cam1_corners.csv> \
  --corner-poses <cam0_corner_poses.csv> --kalibr-result <kalibr-results-imucam.txt> \
  --out-dir <run_dir>
```
...
这轮诊断把故事闭上了:最初 TUM 没追上,不是 pkl/bag/euroc 读取问题,也不是多相机内外参没接上,更不是 `M_a/M_g/A_g/C_g` 扩展 IMU 前向公式错。真正问题是 20/10 kps 的 Ceres pose/bias 轨迹太低频;高 kps 足够,但必须先固定全局块。这个流程已固化到 `tools/run_ceres_two_stage.py`:第一阶段解全局标定和扩展 IMU intrinsic,第二阶段从第一阶段 result 重建状态,固定全局块,用 100/50 kps 细化 pose/bias 并输出最终 residual 与 IMU diagnostics。
...
- **从零独立精度(表1)**:当前代码的 production rerun 暴露了回归。April 组仍是正常独立收敛(1.3–3.1 mm、reproj 0.170–0.208 px),但 March 组全部 151 it `NO_CONVERGENCE`,外参平移 140–154 mm、reproj 2.975–28.162 px。旧的"独立口径 1.3–4.7 mm、reproj 追平 Kalibr"不能作为当前生产结论。
- **扩展 IMU / 多相机 / 输入格式**:`scale-misalignment` 在三组生产数据上已做 Kalibr-vs-Ceres 全量对比,`M_a/M_g` 相对差是 `1e-3` 量级,外参差为 `0.053-0.082°` / `1.85-6.13 mm`;TUM 双目也验证了 multi-camera 和 `M_a/M_g/A_g/C_g` 一致性。Ceres 主程序读统一 CSV/YAML,不依赖 Kalibr Docker 求解;`prepare_ceres_inputs.py` 支持 pkl、ROS bag、EuRoC/TUM `mav0` 转换,可 `--run-calibration` 一键转完直接跑单阶段标定,也可 `--run-two-stage` 一键进入 TUM parity 二阶段流程;转换/角点导出当前仍复用 Kalibr Docker/ROS 环境。
...
- **速度**:当前 independent 表1不能作为干净速度结论:March 组 144–153 s 是失败上限时间,April 组正常收敛为 79–90 s。热启动表2全部正常收敛,为 36–91 s;Kalibr 模拟 Docker 为 145–261 s。注意这不是 native-vs-native——Kalibr 在本机只能 amd64 模拟(有 3–10× 惩罚);要严格倍率需在同一台 Linux 上各跑一次。
...
当前 independent 口径 Ceres(单数据集,把 `<DS>` 换成具体目录):
...
等价 preset:`python3 ceres_cam_imu/tools/run_ceres_sweep.py --preset independent-joint-full --run-name <name>`(默认数据集 cam_imu_2;其它数据集用上面的显式命令)。本轮批量重跑额外加了热启动变体:

```bash
python3 ceres_cam_imu/tools/run_ceres_sweep.py \
  --dataset <DS> --run-name prod_current_rerun_<dataset> \
  --preset independent-joint-full \
  --variant "hotstart-joint-full=--kalibr-corner-defaults --init-from-kalibr --pose-fit-motion-lambda 0.0001 --pose-fit-boundary-anchors --time-shift-prior-sigma 0.0001 --pose-motion-prior --pose-motion-translation-variance 10 --pose-motion-rotation-variance 1 --max-iterations 150 --solver-max-trust-region-radius 10000000 --top-residuals 8" \
  --stop-on-failure
```
