...
当前路线选择是：以 Docker wrapper 作为对外稳定入口，以 forked Kalibr 作为算法核心，把 BenchmarkCalibration `benchmark-reference` 中需要保留的 cam-cam / cam-imu 行为迁移进这个公开可构建的工程。
...
- `cam-imu`：BenchmarkCalibration 兼容的 corner-file 模式，以及 H5 输入模式。
...
`auto` 是 benchmark 推荐路径：先多进程快跑；如果日志出现 rosbag 多进程读取异常，就归档 fast 尝试并自动用单线程重跑。归档文件包括：
...
BenchmarkCalibration `benchmark-reference` 的 cam-imu 离线路径使用 corner-file 模式，并且要匹配以下参数：

- 开启时间偏移估计：`--estimate-time-offset`
- 时间偏移 padding：`--timeoffset-padding 0.04`
- 不导出 poses：`--no-export-poses`

一致性验证中，fixture 1 在参数完全匹配时，Docker 和 Benchmark 输出只存在 `1e-12` 量级的浮点末位差异。结论是 cam-imu 算法路径已经可以复现旧 Benchmark 行为；默认参数不同导致的差异不应被误判为算法不一致。
...
3. 本地 BenchmarkCalibration 仓库里的旧 cam-cam Kalibr。

这次修复已经进入 Docker 所用的 forked Kalibr。Docker fast mode 不再复现之前的 rosbag 多进程读包异常，并且在 `cam2cam_clahe` 上提取 `20/20`。

但本地 BenchmarkCalibration 仓库里的 cam-cam Kalibr 代码目前没有 `BagImageDatasetReader.reopen()` 修复。因此，如果直接运行 BenchmarkCalibration 旧 cam-cam fast 路径，它仍可能遇到原来的多进程读 bag 风险，结果可能因为少用图片而和 Docker 不一致。

如果要让 cam-cam 与 BenchmarkCalibration 项目版严格一致，有两条路线：

- 迁移路线：把 `reopen()` 修复同步回 BenchmarkCalibration 的 cam-cam Kalibr，之后用相同 bag、相同 target、相同 camera model、相同 `KALIBR_MANUAL_FOCAL_LENGTH_INIT`、相同是否展示 report 的参数对比。
- 收敛路线：把 Docker fork 作为唯一 benchmark 入口，BenchmarkCalibration 不再直接跑旧 cam-cam Kalibr，只保留上层数据管理或历史脚本。

当前更推荐收敛到 Docker fork。原因是公开 Docker 工程能被外部用户构建、发布和复现；BenchmarkCalibration 只是内部基准口径，不适合作为外部依赖。
...
corner-file 模式更接近 BenchmarkCalibration 当前离线流程。H5 模式适合未来把图像流统一打包。
...
- 发布 `wang121ye/kalibr-camera-calibration:20.04` 到 DockerHub，或发布 `ghcr.io/wangyendt/kalibr-camera-calibration:20.04` 到 GHCR。
...
- 对比 Docker fast、Docker never、Benchmark patched fast。
...
- 如果 Benchmark 未同步 `reopen()` 修复，则不应把它作为 fast 一致性基准。
...
Docker fork 已经具备对外一行运行的基本形态。cam-cam fast 的核心稳定性问题已经修复，cam-imu 已经能复现 Benchmark 兼容路径。
