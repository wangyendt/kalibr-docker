...
现在开始处理最容易混淆的负号。这条规则在第 0 章 0.14 节已经完整推过一遍；本节在优化语境里重走一次是有意为之——它是后面每一章 Jacobian 符号的出发点，值得在进入 Gauss-Newton 之前再确认一遍。
...
### 3.9.1 旁注：LM damping 不是 residual 权重

上一段顺带出现了“LM damping / trust region”。它和 robust weight 处在不同层级，容易混淆，这里先用一小节澄清；只关心 robust kernel 主线的读者可以直接跳到 3.9.2。

LM damping 可以先理解成 Levenberg-Marquardt 里的当前阻尼强度，常用符号就是：
...
### 3.9.2 Kalibr 的权重函数 $w(s)$

回到 robust kernel 主线。Kalibr 源码里的几个 weight policy 直接给出 $w(s)$。设：
...
![M-estimator weight curves](assets/m_estimator_weights.png)
