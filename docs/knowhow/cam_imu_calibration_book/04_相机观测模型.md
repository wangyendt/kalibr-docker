...
实现侧需要注意：`ceres_cam_imu/` 现在把这些组合收敛到统一的 `camera/CameraModel`。它按 Kalibr 的 `camera_model`、`distortion_model`、`intrinsics` 和 `distortion_coeffs` 读取配置，支持 `pinhole+radtan/equidistant/fov/none`、`omni+radtan/none`、`eucm` 和 `ds`。camera residual 只依赖 `projectWithJacobian()` 输出的 $\mathbf J_{\boldsymbol\pi,\mathbf p_c}$，因此相机模型扩展不会改变外参、time shift 或 pose spline 的链式结构。当前 Ceres 版本仍固定相机内参，不把 $\boldsymbol\eta$ 作为优化变量。
